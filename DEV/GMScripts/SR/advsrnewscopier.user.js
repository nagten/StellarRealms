// ==UserScript==
// @name          ADV SR News Copier
// @namespace     tag:ADVSRNewsCopier
// @description   Add functions to Stellar-Realms News page for easy data sharing. (v1.0) based on srnewscopier
...
(function() {
	...
	* http://www.alignus.com/Cabal/scout.php
	*/
	function scoutToDatabase(ev) {
		var request = extractScoutMsg(ev.currentTarget.parentNode,'<br>');
		request = 'http://www.alignus.com/Cabal/scout_internals.php?action=input&report=' + encodeURI(request);
		GM_xmlhttpRequest({
			method: 'GET',
			url: request,
		...
		if (checkboxes) 
		{
			...
			{			
				if (checkboxes[i].checked)
				{
					...
					if (checkboxes[i].parentNode.parentNode.childNodes[3].childNodes[8].nodeValue.match(/reconnoiter structures/))
					{
						var request = extractScoutMsg(checkboxes[i].parentNode.parentNode.childNodes[3].childNodes[8].parentNode,'<br>');		
						request = 'http://www.alignus.com/Cabal/scout_internals.php?action=input&report=' + encodeURI(request);
